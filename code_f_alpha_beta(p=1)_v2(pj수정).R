...
est_beta <- function(X_b,Z_b,D_b,alpha_b,f_b,h_b)
{
  ...
  {
    hatb[j] = optimize(LL_f,t1=tg[j],lower=0,upper=5)$minimum
  }
  return(hatb)
}
######################################
  ...
  Dis_P2 <- function(eta,T0,D0,X0)
  {
    ...
    for (k in seq(1,length(s_T)-1))
    {
      temp =  
        (D0!=2)*(s_T[k]<=T0)*c_m[k,]
      + (D0==2)*c_m[k,]
      - (D0!=2)*(eta<=T0)*c_m[length(s_T),]
      - (D0==2)*c_m[length(s_T),]
      p[k] = 1/(1/eta + exp(alpha)*sum(temp))
    }
    return(append(p,eta))
  }
...
est_alpha <- function(X_a,Z_a,D_a,beta_a,f_a)
{
  ...
  {
    ll1[i] = (D_a[i]!=2)*sum( exp(X_a[i]*beta_a(s_T))*(s_T<Z_a[i])*f_a )
    ll2[i] = (D_a[i]==2)*sum( exp(X_a[i]*beta_a(s_T))*f_a )
    s0[i] = ll1[i]+ll2[i]
  }
